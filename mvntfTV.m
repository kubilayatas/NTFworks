function [A,B,C,U,V,E]=mvntfTV(X,A,B,C,lambda,mu,R,L,LiDAR_coeff)
...
while 1
    objOld=objNew;
    %%%%%%update A
    M=myKr(C,B,ones(1,R),ones(1,R)*L); % +LiDAR*B       ./+A*B'*B
    A=A.*(X1*M+mu*U+delta*ones(I,J)*B)./(A*M'*M+mu*A+delta*A*B'*B+1e-4);
    %%%%%%update B
    M=myKr(C,A,ones(1,R),ones(1,R)*L); %+LiDAR'*A      ./+B*A'*A    
    ...
    for r=1:R
		...
		[E{r},obj]= prox_TV(LiDAR_coeff.*(Ur*Vr'),lambda/mu,param2);
